...
function makeGuess(){
    ...
    if (userGuess > answer){
        ...
        if(score == 1){
            msg.textContent = "WOW! You guessed the number " + answer + " first try! Amazing! Press play to play again"
            updateScore();
            reset();
        }else{
            msg.textContent = "Congrats! You guessed the number " + answer + " in " + score + " tries! Press play to play again"
            updateScore();
            reset();
        }
    }
